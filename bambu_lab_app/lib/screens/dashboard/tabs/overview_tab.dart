/// 设备总览 Tab（使用 flutter_neumorphism_ui）
library;
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        _ProgressCard(printer: printer, c: c),
        const SizedBox(height: 10),
        _TempRow(s: s, c: c),
        const SizedBox(height: 10),
        _ControlCard(printer: printer, c: c),
      ],
    );
  }
}
...
class _ProgressCard extends StatelessWidget {
  const _ProgressCard({required this.printer, required this.c});
  final PrinterProvider printer;
  final NeuoColors c;
  ...
  Widget build(BuildContext context) {
    ...
    final isPrinting = s.printStatus == PrintStatus.printing || s.printStatus == PrintStatus.pausedUser;
    ...
    return FlutterNeumorphism(
      ...
      child: Column(children: [
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (isPrinting)
            _PreviewThumbnail(
              ...
              error: error,
              ftpOk: ftpOk,
              onRetry: () => printer.fetchPreviewImage(),
              c: c,
            ),
        ]),
        const SizedBox(height: 10),
        ...
        Row(children: [
          const Spacer(),
          if (running) ...[
            Icon(LucideIcons.clock, size: 12, color: c.textSecondary),
            const SizedBox(width: 4),
            Text(s.remainingTimeFormatted, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: c.textPrimary)),
            const SizedBox(width: 10),
          ],
          Text(running ? '$pct%' : '--', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: running ? c.accent : c.textSecondary)),
        ]),
      ]),
    );
  }
...
class _PreviewThumbnail extends StatelessWidget {
    ...
    if (error != null || !ftpOk) {
      return GestureDetector(
        ...
        child: Container(
          ...
          height: size,
          color: c.background.withValues(alpha: 0.5),
          child: Icon(LucideIcons.refreshCw, size: 20, color: c.accent.withValues(alpha: 0.7)),
        ),
      );
    }
  ...
  Widget _placeholder(double size) => Container(
    ...
    height: size,
    color: c.background.withValues(alpha: 0.5),
    child: Icon(LucideIcons.image, size: 20, color: c.textSecondary.withValues(alpha: 0.4)),
  );
}
...
class _TempRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    if (s.chamberTemp != null) ...[
      const SizedBox(width: 8),
      Expanded(child: _temp('箱体', s.chamberTemp, LucideIcons.home, _green, c)),
    ],
  ]);

  Widget _temp(String label, double? t, IconData icon, Color color, NeuoColors c) => FlutterNeumorphism(
    style: NeumorphismStyle(
      color: c.background,
      borderRadius: 12,
      depth: 5,
...
class _ControlCard extends StatelessWidget {
  const _ControlCard({required this.printer, required this.c});
  final PrinterProvider printer;
  final NeuoColors c;
    ...
    return GestureDetector(
      ...
      child: FlutterNeumorphism(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(widget.label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: widget.color)),
        ]),
      ),
    );
  }
...
class _LightRowState extends State<_LightRow> {
  ...
  @override
  Widget build(BuildContext context) {
    final on = widget.printer.state.lightOn;
    return Row(children: [
      Icon(LucideIcons.lightbulb, size: 14, color: on ? Colors.amber : widget.c.textSecondary),
      const SizedBox(width: 6),
      Text('灯光', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: widget.c.textPrimary)),
      const Spacer(),
      GestureDetector(
        onTapDown: (_) => setState(() => _pressed = true),
        onTapUp: (_) {
          setState(() => _pressed = false);
          widget.printer.toggleLight();
        },
        onTapCancel: () => setState(() => _pressed = false),
        child: FlutterNeumorphism(
          style: NeumorphismStyle(
          ...
          child: Text(on ? '开' : '关', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: on ? Colors.amber : widget.c.textSecondary)),
        ),
      ),
    ]);
  }
